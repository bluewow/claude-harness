...
options:
  - label: "승인"
    description: "기획 승인 → 바로 개발 시작"
  - label: "수정 필요"
    description: "수정 사항과 함께 개발 시작"
  - label: "폐기"
    description: "이 태스크를 폐기합니다"
```

### 승인 시
1. history.json에 승인 로그 추가
2. 다음을 안내하고 종료:
```
승인 완료! /dev [ID]-[한국어slug] 를 바로 실행합니다.
```
**중요**: Agent 종료 시 반환 메시지에 반드시 `__AUTO_DEV__:[ID]-[한국어slug]` 를 포함할 것. 메인 대화에서 이 마커를 감지하여 자동으로 /dev 를 실행한다.

### 수정 필요 시
1. AskUserQuestion으로 수정할 내용을 입력받음:
```
question: "어떤 부분을 수정해야 하나요?"
```
2. 사용자의 수정 피드백을 받음
3. history.json에 승인 로그 추가 (수정 사항 포함)
4. 다음을 안내하고 종료:
```
수정 사항과 함께 /dev [ID]-[한국어slug] 를 바로 실행합니다.
```
**중요**: Agent 종료 시 반환 메시지에 반드시 다음 두 가지를 포함할 것:
- `__AUTO_DEV__:[ID]-[한국어slug]`
- `__DEV_MODIFICATIONS__:[사용자가 입력한 수정 사항 전문]`
...
## Agent 종료 후 자동 /dev 실행

PM Agent의 반환 메시지에 `__AUTO_DEV__:[태스크ID]` 패턴이 포함되어 있으면,
메인 대화에서 즉시 `/dev [태스크ID]`를 실행한다 (Skill 도구 사용).

처리 순서:
1. Agent 반환 메시지에서 `__AUTO_DEV__:` 뒤의 태스크 ID를 추출
2. `__DEV_MODIFICATIONS__:` 패턴이 있으면 수정 사항도 추출
3. 사용자에게 안내:
   - 수정 사항 없음: "승인 완료! 백그라운드에서 개발을 시작합니다."
   - 수정 사항 있음: "수정 사항과 함께 개발을 시작합니다."
4. `/dev [태스크ID]` 실행 (Skill 도구) — 수정 사항이 있으면 dev Agent 프롬프트에 포함
